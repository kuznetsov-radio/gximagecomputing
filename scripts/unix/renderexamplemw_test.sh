...
usage() {
  cat <<'EOF'
Usage: renderexamplemw_test.sh [options]
...
Options:
  ...
  --frequencies-ghz F0 [F1 ...]
                            Explicit MW frequency list override in GHz
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --b0sun-deg)
      ...
      shift 2
      ;;
    --frequencies-ghz|--freqlist-ghz)
      shift
      [[ $# -ge 1 ]] || { echo "ERROR: --frequencies-ghz requires at least one value"; exit 1; }
      while [[ $# -gt 0 && "$1" != --* ]]; do
        FREQUENCIES_GHZ+=("$1")
        shift
      done
      [[ ${#FREQUENCIES_GHZ[@]} -gt 0 ]] || { echo "ERROR: --frequencies-ghz requires at least one value"; exit 1; }
      ;;
    --model-path)
...
if [[ ${#FREQUENCIES_GHZ[@]} -gt 0 ]]; then
  ARGS+=(--frequencies-ghz "${FREQUENCIES_GHZ[@]}")
fi
