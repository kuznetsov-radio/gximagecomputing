...
2. Load the EBTEL table:<br/>
   ...
   where ebtelfile is the name of the GX Simulator file containing the EBTEL table that defines the DEM.<br/> 
...
3. Load the instrumental response function:<br/>
   ...
   instrument is the name of the chosen instrument. Currently, the following instruments are supported: 'AIA', 'AIA2', 'TRACE', 'SXT', 'SOLO-FSI', 'SOLO-HRI', 'STEREO-A', 'STEREO-B'. Default: 'AIA'. Note that 

5. Define the size and position of the required radio maps, as well as the emission frequencies:<br/>
   simbox=MakeSimulationBox(xc, yc, dx, dy, Nx, Ny, freqlist [, rot=rot])<br/>
   where:<br/>
   xc and yc are the x and y coordinates of the map center (in the helioprojective coordinate system, in arcseconds).<br/>
   dx and dy are the x and y resolutions of the map (in arcseconds).<br/>
